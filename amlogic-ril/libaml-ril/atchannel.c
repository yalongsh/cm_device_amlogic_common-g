...
#define HANDSHAKE_RETRY_COUNT 80
...
static const char * s_finalResponsesError_gsm[] = {
...
static int isFinalResponseError(const char *line)
{
	...
	if(s_arch != RIL_MODEM_ARCH_TYPE_GSM){
	    for (i = 0 ; i < NUM_ELEMS(s_finalResponsesError_gsm) ; i++) {
	        if (strStartsWith(line, s_finalResponsesError_gsm[i])) {
	            return 1;
	        }
    	}
	}
...
static const char * s_finalResponsesNotSupport[] = {
    "COMMAND NOT SUPPORT",
};

static int isFinalResponseNotSupport(const char *line)
{
    size_t i;

    for (i = 0 ; i < NUM_ELEMS(s_finalResponsesNotSupport) ; i++) {
        if (strStartsWith(line, s_finalResponsesNotSupport[i])) {
            return 1;
        }
    }
...
static const char * s_Unsoliciteds[] = {
    "^HRSSILVL:",
    "^MODE:",
    "^SIMST:",
    "^DSDORMANT:",
...
static int isUnsolicited(const char *line)
{
    size_t i;

    for (i = 0 ; i < NUM_ELEMS(s_Unsoliciteds) ; i++) {
        if (strStartsWith(line, s_Unsoliciteds[i])) {
            return 1;
        }
    }
...
static void processLine(const char *line)
{
    ...
    if (sp_response == NULL || isUnsolicited(line)) {
        /* no command pending */
        ...
        handleFinalResponse(line);
    } else if (isFinalResponseNotSupport(line)) {
        sp_response->success = 0;
		sp_response->atCmdNotSupport = 1;
        handleFinalResponse(line);
...
static const char *readline()
{
    ...
    while (p_eol == NULL) {
        ...
        } while ( s_fd>0 && count < 0 && errno == EINTR);
